...
`detectCapabilitiesWithClaude()` in `lib/capabilities/detection.ts` uses Claude to semantically analyze text and detect capability needs. Unlike pattern-based detection, it understands intent and avoids false positives.
...
async function detectCapabilitiesWithClaude(
  text: string,
...
- Sends existing skill names + categories to Claude (compact summary, not full content)
- Claude returns JSON with matched existing skills and proposed new capabilities
- Validates returned skill names against actual database entries
- Falls back to empty arrays on failure
- Used by `POST /api/tasks/{id}/optimize-field` during task field optimization
