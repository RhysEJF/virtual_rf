...
export function CommitSettingsSection({
  ...
  const [localConfig, setLocalConfig] = useState<CommitSettingsConfig>(config);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        toast({ type: 'success', message: 'Commit settings updated' });
        setHasChanges(false);
        onUpdate?.();
      } else {
  ...
  const handleTargetChange = (key: keyof CommitSettingsConfig, value: SaveTarget) => {
      ...
      <CardHeader>
        <div>
          ...
          <p className="text-text-tertiary text-sm mt-1">
            Where to commit outputs, skills, tools, and files
          </p>
        </div>
        {hasChanges && (
