---
title: "Ralph Setup Skill"
description: "\"I want to set up this project using the Ralph Wiggum AI Safety Monitoring System. Please analyze my project idea, break it down into a `plans/prd.json`, and generate the necessary scaffolding files (`plans/ralph_monitored.sh`, `plans/ralph_guardian.sh`, `plans/recovery_manager.sh`, `CLAUDE.md`, `init.sh`). Include the full AI safety monitoring system that gives Ralph complete autonomy with invisible malicious behavior detection. Identify any missing API keys I need to provide. Do not start coding the app yet, just set up the monitored loop.\""
---
