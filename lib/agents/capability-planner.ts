 ...
 * then creates capability tasks to build them before execution.
 */
...
import type { Task, Intent, CapabilityType } from '../db/schema';
...
/** @deprecated Use CapabilityNeed instead */
export type InfrastructureNeed = CapabilityNeed;
/** @deprecated Use CapabilityPlan instead */
export type InfrastructurePlan = CapabilityPlan;
...
/**
 * Analyze an approach document for capability requirements.
 * Uses pattern matching and optionally Claude for complex cases.
 */
export async function analyzeApproachForCapabilities(
  approach: string,
  intent: Intent | null,
  outcomeId: string
  ...
  return {
    ...
    hasCapabilities: uniqueNeeds.length > 0,
  };
}

/** @deprecated Use analyzeApproachForCapabilities instead */
export const analyzeApproachForInfrastructure = analyzeApproachForCapabilities;
...
/**
 * Use Claude to extract capability needs when pattern matching isn't enough
 */
...
If no capability needs are found, output: NO_CAPABILITIES
...
Be specific about file paths (e.g., skills/market-research.md or tools/web-scraper.ts)`;
  ...
  try {
    ...
    if (!result.success || !result.text || result.text.includes('NO_CAPABILITIES')) {
      return [];
    }
    ...
    console.error('[Capability Planner] Claude extraction failed:', error);
    return [];
  }
}
...
/**
 * Create capability tasks for each need
 */
export function createCapabilityTasks(
  outcomeId: string,
  plan: CapabilityPlan
): Task[] {
  const tasks: Task[] = [];
  ...
  for (let i = 0; i < plan.needs.length; i++) {
    ...
    // Capability tasks get low priority numbers (0-10) so they run first
    const priority = i + 1;
    ...
    const task = createTask({
      ...
      title: `[Capability] Build ${need.type}: ${need.name}`,
      description: `${need.description}\n\nOutput path: ${outputPath}\n\nSpecification:\n${need.specification}`,
      prd_context: JSON.stringify({
        type: 'capability',
        capability_type: need.type,
        path: need.path,
      }),
      priority,
      phase: 'capability',
  ...
  // Mark outcome as needing capability phase
  if (tasks.length > 0) {
...
/** @deprecated Use createCapabilityTasks instead */
export const createInfrastructureTasks = createCapabilityTasks;

/**
 * Check if an outcome has pending capability needs
 */
export function hasCapabilityNeeds(approach: string): boolean {
  const patterns = [
    /skills\/[\w-]+\.md/i,
    /tools\/[\w-]+\.(ts|js)/i,
    /skill[- ]?based/i,
...
/** @deprecated Use hasCapabilityNeeds instead */
...
function deduplicateNeeds(needs: CapabilityNeed[]): CapabilityNeed[] {
