...
- [x] Auto-resolve mode for escalations (`lib/homr/auto-resolver.ts`)
  ...
  - Heuristic-based resolution for security (never), failure (retry once), and ambiguity (always human)
  - Complexity escalations evaluated by Claude with task-type awareness (research→more turns, coding→decomposition)
  - Auto-spawned workers disable complexity check to prevent re-escalation cascades
